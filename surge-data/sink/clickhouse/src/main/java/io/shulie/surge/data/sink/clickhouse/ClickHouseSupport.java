...
public class ClickHouseSupport implements Lifecycle, Stoppable {
    ...
    public ClickHouseSupport(@Named("config.clickhouse.url") String url,
        ...
        try {
            ...
            if (enableRound) {
                clickHouseDataSource = new RoundClickhouseDataSource(url, clickHouseProperties, false);
            } else {
    ...
    public synchronized void start() {
        ...
        rotationSqlBatch = new RotationBatch(new CountRotationPolicy(batchCount), new TimedRotationPolicy(1, TimeUnit.SECONDS));
     ...
     * @param sql
     */
    public synchronized void batchUpdate(final String... sql) {
     ...
     * @param batchArgs
     */
    public synchronized void batchUpdate(final String sql, final List<Object[]> batchArgs) {
...
/*
                    System.out.println("save batch :" + JSON.toJSON(batchSql));
*/
