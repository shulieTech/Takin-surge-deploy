 ...
 */
public class GrpcSupplierObserver implements LifecycleObserver {
    private static final Logger logger = LoggerFactory.getLogger(GrpcSupplierObserver.class);
    ...
    public void afterStart(Lifecycle target) {
        GrpcSupplier grpcSupplier = (GrpcSupplier) target;
        if (registerToGateWay != null && registerToGateWay) {
            //调用网关服务注册
            registerToGateWay(IpAddressUtils.getLocalAddress(), grpcSupplier.getPort());
        }
    }
