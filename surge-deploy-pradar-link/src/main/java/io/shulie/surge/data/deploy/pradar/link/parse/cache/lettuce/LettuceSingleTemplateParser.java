...
import com.pamirs.attach.plugin.dynamic.template.RedisTemplate.LettuceSingleTemplate;
...
public class LettuceSingleTemplateParser extends AbstractTemplateParser {
    ...
    public Class<?> supportTemplateClass() {
        return LettuceSingleTemplate.class;
    }

    @Override
    public ShadowDatabaseParseResult doParseTemplate(TTrackClickhouseModel traceModel,
        TemplateEnum templateEnum) {
        Attachment<LettuceSingleTemplate> attachment = JSON.parseObject(
            TemplateParseHandler.detachAttachment(traceModel),
            new TypeReference<Attachment<LettuceSingleTemplate>>() {});
        LettuceSingleTemplate template = attachment.getExt();
