...
public class RedissionSingleTemplateParser extends AbstractTemplateParser {
    ...
    public Class<?> supportTemplateClass() {
        return RedissionSingleTemplate.class;
    }

    @Override
    public ShadowDatabaseParseResult doParseTemplate(TTrackClickhouseModel traceModel,
        TemplateEnum templateEnum) {
        Attachment<RedissionSingleTemplate> attachment = JSON.parseObject(
            TemplateParseHandler.detachAttachment(traceModel),
            new TypeReference<Attachment<RedissionSingleTemplate>>() {});
        RedissionSingleTemplate template = attachment.getExt();
