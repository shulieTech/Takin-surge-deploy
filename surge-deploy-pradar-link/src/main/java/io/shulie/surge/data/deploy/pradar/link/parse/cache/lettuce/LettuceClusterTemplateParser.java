...
import com.pamirs.attach.plugin.dynamic.template.RedisTemplate.LettuceClusterTemplate;
...
public class LettuceClusterTemplateParser extends AbstractTemplateParser {
    ...
    public Class<?> supportTemplateClass() {
        return LettuceClusterTemplate.class;
    }

    @Override
    public ShadowDatabaseParseResult doParseTemplate(TTrackClickhouseModel traceModel,
        TemplateEnum templateEnum) {
        Attachment<LettuceClusterTemplate> attachment = JSON.parseObject(
            TemplateParseHandler.detachAttachment(traceModel),
            new TypeReference<Attachment<LettuceClusterTemplate>>() {});
        LettuceClusterTemplate template = attachment.getExt();
