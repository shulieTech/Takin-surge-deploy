...
import com.pamirs.attach.plugin.dynamic.template.RedisTemplate.JedisMasterSlaveTemplate;
...
public class JedisMasterSlaveTemplateParser extends AbstractTemplateParser {
    @Override
    public Class<?> supportTemplateClass() {
        return JedisMasterSlaveTemplate.class;
    }

    @Override
    public ShadowDatabaseParseResult doParseTemplate(TTrackClickhouseModel traceModel,
        TemplateEnum templateEnum) {
        Attachment<JedisMasterSlaveTemplate> attachment = JSON.parseObject(
            TemplateParseHandler.detachAttachment(traceModel),
            new TypeReference<Attachment<JedisMasterSlaveTemplate>>() {});
        JedisMasterSlaveTemplate template = attachment.getExt();
