...
CREATE TABLE t_pressure on cluster ck_cluster
(
    `appName`              String,
    `entranceId`           Nullable(String),
    `entranceNodeId`       Nullable(String),
...
)ENGINE = ReplacingMergeTree PARTITION BY taskId ORDER BY (taskId,traceId,cost,startDate,appName,rpcId) PRIMARY KEY (taskId,traceId,cost,startDate,appName,rpcId) TTL startDate + toIntervalDay(7) SETTINGS index_granularity = 8192;

CREATE TABLE t_trace_pressure on cluster ck_cluster as t_pressure ENGINE = Distributed(ck_cluster, default, t_pressure, sipHash64(taskId));
...
CREATE TABLE t_pressure on cluster ck_cluster( `appName` String, `entranceId` Nullable(String), `entranceNodeId` Nullable(String), `traceId` String, `level` Nullable(Int8), `parentIndex` Nullable(Int8), `index` Nullable(Int8), `rpcId` String, `rpcType` Int8, `logType` Nullable(Int8), `traceAppName` Nullable(String), `upAppName` Nullable(String), `startTime` Int64, `cost` Int32, `middlewareName` Nullable(String), `serviceName` Nullable(String), `methodName` Nullable(String), `remoteIp` Nullable(String), `port` Nullable(Int32), `resultCode` Nullable(String), `requestSize` Nullable(String), `responseSize` Nullable(String), `request` Nullable(String), `response` Nullable(String), `clusterTest` Nullable(String), `callbackMsg` Nullable(String), `samplingInterval` Nullable(String), `localId` Nullable(String), `attributes` Nullable(String), `localAttributes` Nullable(String), `async` Nullable(String), `version` Nullable(String), `hostIp` Nullable(String), `agentId` Nullable(String), `startDate` DateTime, `createDate` DateTime DEFAULT toDateTime(now()), `timeMin` Nullable(Int64) DEFAULT 0, `entranceServiceType` Nullable(String), `parsedServiceName` String, `parsedMethod` String, `parsedAppName` Nullable(String), `parsedMiddlewareName` Nullable(String), `parsedExtend` Nullable(String), `dateToMin` Int64, taskId String, flag Nullable(String), flagMessage Nullable(String), receiveTime Nullable(Int64), processTime Nullable(Int64), saveCkTime DateTime DEFAULT now(), `userAppKey` String, `envCode` String, `userId` String,INDEX ix_userAppKey userAppKey TYPE set(1000000) GRANULARITY 5,INDEX ix_traceid traceId TYPE set(0) GRANULARITY 5,INDEX ix_appName appName TYPE set(0) GRANULARITY 5,INDEX ix_parsedServiceName parsedServiceName TYPE set(0) GRANULARITY 5,INDEX ix_parsedMethod parsedMethod TYPE set(0) GRANULARITY 5,INDEX ix_cost cost TYPE minmax GRANULARITY 5 )ENGINE = ReplacingMergeTree PARTITION BY taskId ORDER BY (taskId,traceId,cost,startDate,appName,rpcId) PRIMARY KEY (taskId,traceId,cost,startDate,appName,rpcId) TTL startDate + toIntervalDay(7) SETTINGS index_granularity = 8192;
CREATE TABLE t_trace_pressure on cluster ck_cluster as t_pressure ENGINE = Distributed(ck_cluster, default, t_pressure, sipHash64(taskId));
