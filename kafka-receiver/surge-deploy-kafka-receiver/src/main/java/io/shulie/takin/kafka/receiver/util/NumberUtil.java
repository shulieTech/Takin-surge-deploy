...
public class NumberUtil {
    ...
    public static <T> BigDecimal maxBigDecimal(Collection<T> list, Function<T, BigDecimal> func) {
        return maxBigDecimal(list, func, new BigDecimal("0.00"));
    }
    public static <T> BigDecimal maxBigDecimal(Collection<T> list, Function<T, BigDecimal> func, BigDecimal defValue) {
        if (CollectionUtils.isEmpty(list)) {
            return defValue;
        }
        OptionalDouble maxOpt = list.stream().filter(Objects::nonNull)
                ...
                .max();
        return maxOpt.isPresent() ? BigDecimal.valueOf(maxOpt.getAsDouble()) : defValue;
    }
    ...
    public static <T> BigDecimal minBigDecimal(Collection<T> list, Function<T, BigDecimal> func) {
        return minBigDecimal(list, func, new BigDecimal("0.00"));
    }
    public static <T> BigDecimal minBigDecimal(Collection<T> list, Function<T, BigDecimal> func, BigDecimal defValue) {
        if (CollectionUtils.isEmpty(list)) {
            return defValue;
        }
        OptionalDouble minOpt = list.stream().filter(Objects::nonNull)
                ...
                .mapToDouble(BigDecimal::doubleValue)
                .min();
        return minOpt.isPresent() ? BigDecimal.valueOf(minOpt.getAsDouble()) : defValue;
    }
    ...
    public static <T> BigDecimal sumBigDecimal(Collection<T> list, Function<T, BigDecimal> func) {
        return sumBigDecimal(list, func, new BigDecimal("0.00"));
    }

    public static <T> BigDecimal sumBigDecimal(Collection<T> list, Function<T, BigDecimal> func, BigDecimal devValue) {
        if (CollectionUtils.isEmpty(list)) {
            return devValue;
        }
        double sum = list.stream().filter(Objects::nonNull)
                .map(func)
                .filter(Objects::nonNull)
