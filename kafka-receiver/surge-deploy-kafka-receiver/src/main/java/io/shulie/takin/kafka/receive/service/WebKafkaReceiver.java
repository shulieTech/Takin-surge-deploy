...
import io.shulie.takin.kafka.receive.constant.ContextSourceEnum;
import io.shulie.takin.kafka.receive.constant.TakinClientAuthConstant;
...
public class WebKafkaReceiver implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ScheduledExecutorService kafkaReceivePool = Executors.newScheduledThreadPool(10, new NamedThreadFactory("kafka_receive", true));
...
//                    }
        ...
        kafkaReceivePool.execute(() -> {
            ...
            messageReceiveService.receive(ListUtil.of("stress-test-agent-api-register"), new MessageReceiveCallBack() {
                @Override
                public void success(MessageEntity messageEntity) {
                    iApplicationApiManageService.dealMessage(JSONObject.toJSONString(messageEntity.getBody()), dealHeader(messageEntity.getHeaders()));
                }
