...
public class E2EDefaultDigester implements DataDigester<RpcBased> {
    ...
    @Inject
    private E2ETraceMetricsResultListener traceMetricsResultListener;
    ...
    private DefaultAggregator e2eAggregator;
    ...
    public void digest(DigestContext<RpcBased> context) {
        if (isRunning.compareAndSet(false, true)) {
            try {
                defaultAggregator = new DefaultAggregator(5, 60);
                defaultAggregator.addListener(PradarRtConstant.E2E_METRICS_ID_TRACE, e2EMetricsResultListener);
                defaultAggregator.start();

                e2eAggregator = new DefaultAggregator(5, 60);
                e2eAggregator.addListener(PradarRtConstant.E2E_METRICS_ID_TRACE, traceMetricsResultListener);
                e2eAggregator.start();
            } catch (Exception e) {
        ...
        AggregateSlot<Metric, CallStat> e2eSlot = e2eAggregator.getSlotByTimestamp(timeStamp);
        ...
        try {
            ...
            slot.addToSlot(Metric.of(PradarRtConstant.METRICS_ID_TRACE, tags.toArray(new String[0]), "", new String[]{}), callStat);
        } catch (Exception e) {
